...
class ChampionChallengerManager:
    ...
    def register_staging_model(
        ...
        try:
            ...
            logger.info("Model %s registered to STAGING", model_version)
            return staging_metadata
        except Exception as e:
            logger.error("Failed to register staging model: %s", e)
            raise
    ...
    def evaluate_staging_model(
        ...
        try:
            ...
            evaluation_result = {
                ...
                "gains": {"accuracy_gain": accuracy_gain, "f1_gain": f1_gain},
                "passes_threshold": {
            ...
            logger.info("Evaluation completed for %s", staging_model_version)
            return evaluation_result
        except Exception as e:
            logger.error("Evaluation failed: %s", e)
            raise

    def request_approval(self, staging_model_version: str, requestor: str = "system") -> Dict:
        try:
            eval_path = f"{self.model_registry_path}/staging/{staging_model_version}/evaluation.json"
            ...
            logger.info("Approval request created for %s", staging_model_version)
            return approval_request
        except Exception as e:
            logger.error("Failed to create approval request: %s", e)
            raise

    def approve_model_promotion(self, staging_model_version: str, approver: str, comments: str = None) -> Dict:
        try:
            approval_path = f"{self.model_registry_path}/staging/{staging_model_version}/approval.json"
            ...
            logger.info("Model %s promoted to PRODUCTION by %s", staging_model_version, approver)
            return prod_metadata
        except Exception as e:
            logger.error("Failed to approve model promotion: %s", e)
            raise

    def reject_model_promotion(self, staging_model_version: str, approver: str, reason: str) -> Dict:
        try:
            approval_path = f"{self.model_registry_path}/staging/{staging_model_version}/approval.json"
            with open(approval_path, "r") as f:
                approval = json.load(f)
            ...
            logger.info("Model %s rejected by %s", staging_model_version, approver)
            return approval
        except Exception as e:
            logger.error("Failed to reject model: %s", e)
            raise
    ...
    def rollback_to_archived(self, archived_model_version: str) -> Dict:
        try:
            ...
            logger.info("Rollback completed to %s", archived_model_version)
    ...
    def _get_current_production_model(self) -> Optional[str]:
        ...
        if not os.path.exists(prod_dir):
            return None
        models = [d for d in os.listdir(prod_dir) if os.path.isdir(os.path.join(prod_dir, d))]
        if not models:
            return None
    ...
    def _list_models_by_status(self, status: ModelStatus) -> List[Dict]:
        ...
        for model_name in os.listdir(status_dir):
            ...
            if os.path.isdir(model_path):
                models.append({"version": model_name, "path": model_path, "status": status.value})
