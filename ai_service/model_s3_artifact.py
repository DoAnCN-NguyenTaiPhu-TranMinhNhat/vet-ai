"""
Optional S3 backup for trained model directories (Pha B — model storage).

Set VETAI_MODEL_ARTIFACT_S3_BUCKET to enable upload after training and download
before set_active when the version folder is missing locally (e.g. new pod).
"""
...
def ensure_model_directory_from_s3(model_version: str, clinic_key: Optional[str] = None) -> Tuple[bool, Optional[str]]:
    """
    If local version dir is missing required artifacts, try to download from S3.
    Returns (ok, error_message) where ok means resolve_model_dir has model.pkl at least.
    """
