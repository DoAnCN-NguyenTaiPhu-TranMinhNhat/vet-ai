"""
Đối chiếu run MLflow gần nhất với model đang active trên registry (global / theo clinic).
Dùng tag chuẩn hóa từ training_engine.log_to_mlflow (vetai_*).
"""
...
from ai_service.model_registry import get_active_model, get_active_model_for_clinic
...
def compare_latest_run_to_active(clinic_id: Optional[str] = None) -> Dict[str, Any]:
    """
    Trả về run MLflow mới nhất trong experiment tương ứng scope và so sánh với model đang serve.
    """
    ...
    try:
        ...
        if run_mv and active_version:
            ...
            out["note"] = (
                "Run mới nhất không có tag vetai_model_version (run cũ trước khi chuẩn hóa tag)."
            )
        else:
            out["versions_match"] = None
